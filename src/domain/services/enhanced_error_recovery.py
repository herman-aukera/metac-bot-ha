...
class DiscreteQuestionForecaster:
    ...
    def forecast_discrete_question(
        ...
        if background_info.strip():
            reasoning_parts.extend([
                "",
                "Background Context:",
                background_info.strip()[:500] + ("..." if len(background_info) > 500 else "")
            ])
        ...
        if research_data.strip():
            reasoning_parts.extend([
                "",
                "Research Findings:",
                research_data.strip()[:500] + ("..." if len(research_data) > 500 else "")
            ])
        ...
        if num_options <= 3:
            ...
            probabilities = self._generate_informed_distribution(options, question_text, research_data)
        ...
        if total_prob > 0:
            probabilities = {opt: prob/total_prob for opt, prob in probabilities.items()}
        else:
    ...
    def _generate_informed_distribution(self, options: List[str], question_text: str, research_data: str) -> Dict[str, float]:
    ...
    def _generate_weighted_distribution(self, options: List[str], question_text: str) -> Dict[str, float]:
        ...
        probabilities = {opt: weight/total_weight for opt, weight in zip(options, weights)}

        return probabilities

    def _generate_conservative_distribution(self, options: List[str]) -> Dict[str, float]:
    ...
    def format_probabilities_for_metaculus(self, probabilities: Dict[str, float]) -> List[Tuple[str, float]]:
...
class EnhancedErrorRecovery:
    ...
    async def handle_forecast_failure(
        ...
        logger.warning(f"Forecast failure for {question_id} (attempt {attempt_number}): {error_type}: {error_message}")
        ...
        should_retry = await self._should_retry_error(error, attempt_number, max_attempts)

        if should_retry:
            # Calculate backoff delay
            backoff_delay = await self._calculate_backoff_delay(error_type, attempt_number)
            if backoff_delay > 0:
                logger.info(f"Backing off {backoff_delay}s before retry for {question_id}")
    ...
    async def _should_retry_error(self, error: Exception, attempt_number: int, max_attempts: int) -> bool:
        ...
        if any(phrase in error_message for phrase in [
            ...
            'key limit exceeded', '429', 'rate_limit_exceeded'
        ]):
            return True
        ...
        if any(phrase in error_message for phrase in [
            'invalid api key', 'unauthorized', 'not found',
            'bad request', 'invalid model', 'unsupported'
        ]):
            return False
    ...
    async def _calculate_backoff_delay(self, error_type: str, attempt_number: int) -> float:
        ...
        delay = min(self.max_backoff, base_delay * (self.backoff_multiplier ** (attempt_number - 1)))
    ...
    async def _generate_fallback_forecast(self, question: Any, error: Exception) -> Any:
        ...
        if 'Binary' in question_type:
            ...
            return type('FallbackBinaryResult', (), {
            ...
            try:
                ...
                return date_forecaster.forecast_date_question(
                    ...
                    upper_bound=getattr(question, 'upper_bound', datetime.now() + timedelta(days=365)),
                    research_data="",
                    fine_print=""
                )
            except:
                # Ultimate fallback for date questions
                return type('FallbackDateResult', (), {
                    'reasoning': fallback_reasoning.strip(),
                    'confidence': 0.2,
                    'question_id': question_id,
            ...
            return type('FallbackMultipleChoiceResult', (), {
            ...
            return type('FallbackNumericResult', (), {
            ...
            return type('FallbackErrorResult', (), {
