...
class LogScoringOptimizer:
    ...
    def __init__(self):
        ...
        self.extreme_penalty_threshold = 0.05  # Values closer than this to 0/1 are extreme
    ...
    def calculate_log_score(self, prediction_value: float, actual_outcome: bool) -> float:
    ...
    def simulate_log_scoring_performance(
        ...
        for value in prediction_values:
            ...
            expected_score = (
                outcome_prob * math.log(value) +
                (1 - outcome_prob) * math.log(1 - value)
            )
        ...
        return {
            ...
            "average_score": total_score / len(prediction_values) if prediction_values else 0,
    ...
    def _calculate_expected_log_score(self, prediction_value: float) -> float:
        ...
        expected_score = 0.5 * math.log(prediction_value) + 0.5 * math.log(1 - prediction_value)
    ...
    def _find_optimal_range(self, prediction_value: float, risk_level: str) -> Tuple[float, float]:
    ...
    def _generate_recommendations(self, prediction_value: float, risk_level: str) -> List[str]:
        ...
        if risk_level == "extreme":
            ...
            if prediction_value < 0.1:
                recommendations.append(f"Increase prediction to at least 0.1 (currently {prediction_value:.3f})")
            elif prediction_value > 0.9:
                recommendations.append(f"Decrease prediction to at most 0.9 (currently {prediction_value:.3f})")

        elif risk_level == "high":
            recommendations.append("Consider moving prediction toward safer range (0.2-0.8)")

        elif risk_level == "medium":
            recommendations.append("Prediction is in acceptable range but could be more conservative")
        ...
        if prediction_value > 0.8:
            recommendations.append("High confidence prediction - ensure strong evidence supports this")
        elif prediction_value < 0.2:
            recommendations.append("Low confidence prediction - ensure this reflects true uncertainty")
