...
from ..value_objects.tournament_strategy import TournamentStrategy, CompetitiveIntelligence
...
class TournamentAnalytics:
        ...
        try:
            ...
            for i, participant in enumerate(participants):
                if participant.get("user_id") == our_user_id:
                    ...
                    our_percentile = (total_participants - our_ranking + 1) / total_participants
                    break
            ...
            scores = [p.get("score", 0.0) for p in participants if p.get("score") is not None]
            score_distribution = self._calculate_score_distribution(scores)

            # Identify competitive gaps
            competitive_gaps = self._identify_competitive_gaps(our_score, scores, our_ranking)
            ...
            self.logger.info(f"Analyzed tournament standings for tournament {tournament_id}: "
    ...
    def _analyze_top_performers(self, top_participants: List[Dict[str, Any]]) -> List[CompetitorProfile]:
    ...
    def _analyze_prediction_patterns(self, participant: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _generate_improvement_opportunities(
        ...
        if top_performers:
            ...
            common_strengths = [
                ...
                if count >= len(top_performers) * 0.6  # 60% of top performers have this strength
            ]

            for strength in common_strengths:
                if strength == "excellent_accuracy":
                    opportunities.append("Focus on improving prediction accuracy through better research and reasoning")
                elif strength == "well_calibrated":
                    opportunities.append("Improve confidence calibration through systematic feedback analysis")
                elif strength == "high_volume":
                    opportunities.append("Increase participation rate to answer more questions")
        ...
        if competitive_gaps.get("next_rank", 0) < 5:  # Small gap to next rank
            opportunities.append("Small gap to next rank - focus on consistency to move up")

        if competitive_gaps.get("top_10_percent", 0) > 20:  # Large gap to top 10%
            opportunities.append("Significant improvement needed - consider strategy overhaul")
    ...
    def detect_market_inefficiencies(
        ...
        try:
            inefficiencies = []
            question_id = UUID(question_data.get("id", "00000000-0000-0000-0000-000000000000"))

            # Analyze prediction distribution
            predictions = [p.get("prediction", 0.5) for p in community_predictions if p.get("prediction") is not None]

            if len(predictions) < 5:  # Not enough data
                return inefficiencies

            # Detect overconfidence bias
            overconfidence = self._detect_overconfidence_bias(predictions, question_data)
            if overconfidence:
            ...
            anchoring = self._detect_anchoring_bias(predictions, question_data, historical_patterns)
            if anchoring:
            ...
            momentum = self._detect_momentum_patterns(predictions, community_predictions)
            if momentum:
                inefficiencies.extend(momentum)

            self.market_inefficiencies.extend(inefficiencies)

            self.logger.info(f"Detected {len(inefficiencies)} market inefficiencies for question {question_id}")
    ...
    def _detect_herding_behavior(
        ...
        if prediction_std < 0.05 and 0.2 < prediction_mean < 0.8:  # Tight clustering away from extremes
            ...
            for p in community_predictions:
                if p.get("timestamp"):
                    try:
                        ...
                        if (current_time - timestamp).total_seconds() < 86400:  # Last 24 hours
                            recent_predictions.append(p)
                    except (ValueError, AttributeError):
                        continue

            if len(recent_predictions) > len(community_predictions) * 0.5:  # Most predictions are recent
                return MarketInefficiency(
                    inefficiency_type=MarketInefficiencyType.HERDING_BEHAVIOR,
                    question_id=UUID(community_predictions[0].get("question_id", "00000000-0000-0000-0000-000000000000")),
                    ...
                    metadata={
                        ...
                        "recent_predictions_ratio": len(recent_predictions) / len(community_predictions)
                    }
                )
    ...
    def _detect_anchoring_bias(
        ...
        for round_num in round_numbers:
            ...
            if len(close_predictions) > len(predictions) * 0.4:  # 40% cluster around round number
                return MarketInefficiency(
    ...
    def _detect_recency_bias(
        ...
        try:
            ...
            if older_predictions:
                recent_mean = statistics.mean([p["prediction"] for p in recent_predictions])
                older_mean = statistics.mean([p["prediction"] for p in older_predictions])
            ...
            if len(trend_values) >= 4:
                ...
                if trend_strength > 0.1:
                    inefficiencies.append(MarketInefficiency(
                        ...
                        metadata={"trend_strength": trend_strength, "direction": "upward"}
                    ))
                ...
                # Strong downward momentum
                    ...
                    inefficiencies.append(MarketInefficiency(
                        ...
                        expiration_estimate=datetime.utcnow() + timedelta(days=1),
                        exploitation_strategy="Consider whether momentum will continue or reverse",
                        metadata={"trend_strength": trend_strength, "direction": "downward"}
                    ))
    ...
    def identify_strategic_opportunities(
        ...
        try:
            ...
            timing_opportunities = self._identify_timing_opportunities(question_pipeline, tournament_context)
            opportunities.extend(timing_opportunities)

            # Information edge opportunities
            info_opportunities = self._identify_information_edge_opportunities(question_pipeline, our_performance)
            opportunities.extend(info_opportunities)

            # Competitive positioning opportunities
            positioning_opportunities = self._identify_positioning_opportunities(tournament_context, our_performance)
            opportunities.extend(positioning_opportunities)

            # Resource allocation opportunities
            resource_opportunities = self._identify_resource_allocation_opportunities(question_pipeline, our_performance)
    ...
    def _identify_timing_opportunities(
        ...
        for question in question_pipeline:
            try:
                question_id = UUID(question.get("id", "00000000-0000-0000-0000-000000000000"))
                ...
                time_to_deadline = (deadline_dt - datetime.utcnow()).total_seconds() / 3600  # hours
                ...
                if time_to_deadline > 48 and question.get("prediction_count", 0) < 10:
                    opportunities.append(StrategicOpportunity(
                        ...
                        resource_requirements={"research_time": 2.0, "analysis_depth": 0.8},
                        recommended_actions=[
                ...
                # Late mover advantage
                    ...
                    opportunities.append(StrategicOpportunity(
                        ...
                        resource_requirements={"research_time": 1.5, "analysis_depth": 0.6},
                        recommended_actions=[
        ...
        for question in question_pipeline:
            try:
                ...
                if category in category_performance:
                    ...
                    if cat_perf.get("brier_score", 1.0) < 0.2 and cat_perf.get("question_count", 0) >= 5:
                        opportunities.append(StrategicOpportunity(
                            ...
                            resource_requirements={"research_time": 1.0, "analysis_depth": 0.9},
                            recommended_actions=[
    ...
    def analyze_performance_attribution(
        ...
        try:
            attribution = {
                "accuracy_drivers": self._analyze_accuracy_drivers(our_performance_data, question_history),
                "calibration_analysis": self._analyze_calibration_factors(our_performance_data, question_history),
                "category_performance": self._analyze_category_performance_attribution(our_performance_data, competitor_performance_data),
                "timing_impact": self._analyze_timing_impact(our_performance_data, question_history),
                "confidence_optimization": self._analyze_confidence_optimization(our_performance_data, question_history),
                "competitive_advantages": self._identify_competitive_advantages(our_performance_data, competitor_performance_data),
                "performance_gaps": self._analyze_performance_gaps(our_performance_data, competitor_performance_data)
            }
    ...
    def _analyze_accuracy_drivers(
        ...
        for question in question_history:
            if question.get("research_depth") and question.get("brier_score"):
                ...
                accuracy_scores.append(1 - question["brier_score"])  # Convert to accuracy

        if len(research_scores) >= 3:
            # Simple correlation calculation
            if len(set(research_scores)) > 1:  # Avoid division by zero
                correlation = self._calculate_correlation(research_scores, accuracy_scores)
        ...
        for category, our_perf in our_categories.items():
            ...
            for competitor in competitor_performance:
                ...
                if category in comp_categories:
                    competitor_scores.append(comp_categories[category].get("brier_score", 0.5))
            ...
            if competitor_scores:
                ...
                category_attribution[category] = {
                    ...
                    "relative_performance": avg_competitor_brier - our_brier,  # Positive = we're better
                    "percentile_rank": sum(1 for score in competitor_scores if score > our_brier) / len(competitor_scores),
    ...
    def _analyze_timing_impact(
        ...
        if early_predictions and late_predictions:
            timing_analysis["early_vs_late_performance"] = {
                ...
                "timing_advantage": statistics.mean(early_predictions) - statistics.mean(late_predictions)
            }
        ...
        for bucket, scores in confidence_buckets.items():
            if scores:
                confidence_analysis["optimal_confidence_ranges"][bucket] = {
                    ...
                    "performance_quality": "excellent" if statistics.mean(scores) < 0.2 else
                                         "good" if statistics.mean(scores) < 0.3 else "needs_improvement"
                }
    ...
    def _identify_competitive_advantages(
        ...
        if competitor_performance:
            competitor_briers = [c.get("average_brier_score", 0.5) for c in competitor_performance]
            competitor_calibrations = [c.get("calibration_score", 0.5) for c in competitor_performance]
            competitor_volumes = [c.get("questions_answered", 0) for c in competitor_performance]

            # Accuracy advantage
            if our_brier < statistics.mean(competitor_briers):
                percentile = sum(1 for b in competitor_briers if b > our_brier) / len(competitor_briers)
                advantages.append({
            ...
            if our_calibration > statistics.mean(competitor_calibrations):
                percentile = sum(1 for c in competitor_calibrations if c < our_calibration) / len(competitor_calibrations)
                advantages.append({
            ...
            if our_volume > statistics.mean(competitor_volumes):
                percentile = sum(1 for v in competitor_volumes if v < our_volume) / len(competitor_volumes)
                advantages.append({
    ...
    def _analyze_performance_gaps(
        ...
        if competitor_performance:
            # Find top performers
            top_performers = sorted(competitor_performance, key=lambda x: x.get("average_brier_score", 1.0))[:5]

            if top_performers:
                top_avg_brier = statistics.mean([p.get("average_brier_score", 0.5) for p in top_performers])
                ...
                if our_brier > top_avg_brier:
                    ...
                    gaps.append({
                        ...
                        "severity": "high" if gap_size > 0.1 else "medium" if gap_size > 0.05 else "low",
                        "improvement_potential": gap_size * 100,  # Rough scoring improvement estimate
                        "recommendations": [
                ...
                top_avg_calibration = statistics.mean([p.get("calibration_score", 0.5) for p in top_performers])
                ...
                if our_calibration < top_avg_calibration:
                    ...
                    gaps.append({
                        ...
                        "severity": "high" if gap_size > 0.2 else "medium" if gap_size > 0.1 else "low",
                        "improvement_potential": gap_size * 50,  # Rough scoring improvement estimate
                        "recommendations": [
    ...
    def generate_optimization_recommendations(
        ...
        try:
            ...
            calibration_analysis = performance_attribution.get("calibration_analysis", {})
            ...
            category_performance = performance_attribution.get("category_performance", {})
            for category, perf in category_performance.items():
                if not perf.get("competitive_advantage", False) and perf.get("sample_size", 0) >= 5:
                    recommendations.append({
        ...
        try:
            ...
            report = {
                ...
                "competitive_position": {
                    ...
                    "competitive_momentum": self._assess_competitive_momentum(standings)
                },
                "competitive_landscape": {
                    ...
                    "market_concentration": self._calculate_market_concentration(standings),
                    "competitive_threats": self._identify_competitive_threats(standings)
                },
                "market_analysis": {
                    "inefficiencies_detected": len(recent_inefficiencies),
                    "inefficiency_types": list(set(ineff.inefficiency_type.value for ineff in recent_inefficiencies)),
                    "average_potential_advantage": statistics.mean([ineff.potential_advantage for ineff in recent_inefficiencies]) if recent_inefficiencies else 0.0,
                    "exploitable_opportunities": len([ineff for ineff in recent_inefficiencies if ineff.potential_advantage > 0.1]),
                    "market_efficiency_score": self._calculate_market_efficiency_score(recent_inefficiencies)
                },
                "strategic_opportunities": {
                    "opportunities_count": len(recent_opportunities),
                    "opportunity_types": list(set(opp.opportunity_type.value for opp in recent_opportunities)),
                    "total_potential_impact": sum(opp.potential_impact for opp in recent_opportunities),
                    "high_impact_opportunities": len([opp for opp in recent_opportunities if opp.potential_impact > 0.15]),
                    "time_sensitive_opportunities": len([opp for opp in recent_opportunities if opp.time_sensitivity > 0.7])
                },
    ...
    def _calculate_market_concentration(self, standings: TournamentStandings) -> Dict[str, float]:
        """Calculate market concentration metrics."""
        if not standings.score_distribution:
            return {}

        scores = [standings.score_distribution.get(key, 0) for key in ["max", "q75", "median", "q25", "min"]]
        if not any(scores):
            return {}

        # Calculate concentration metrics
        score_range = standings.score_distribution.get("max", 0) - standings.score_distribution.get("min", 0)
        iqr = standings.score_distribution.get("q75", 0) - standings.score_distribution.get("q25", 0)

        return {
            "score_range": score_range,
            "interquartile_range": iqr,
            "concentration_ratio": iqr / score_range if score_range > 0 else 0,
            "market_tightness": "high" if iqr / score_range < 0.3 else "medium" if iqr / score_range < 0.6 else "low"
        }

    def _identify_competitive_threats(self, standings: TournamentStandings) -> List[Dict[str, Any]]:
            ...
            if performer.current_ranking and performer.total_score:
                ...
                if rank_diff <= 5 and score_diff <= 10:
                    ...
                    threats.append({
                        ...
                        "rank_difference": performer.current_ranking - standings.our_ranking,
                        "score_difference": performer.total_score - standings.our_score,
                        "strengths": performer.strengths,
                        "competitive_advantages": [
                            strength for strength in performer.strengths
                            if strength in ["excellent_accuracy", "well_calibrated", "high_volume"]
                        ]
                    })

        return sorted(threats, key=lambda x: x["rank_difference"])

    def _calculate_market_efficiency_score(self, inefficiencies: List[MarketInefficiency]) -> float:
    ...
    def _generate_strategic_recommendations(
        ...
        if inefficiencies:
            ...
            for ineff in inefficiencies:
                ...
                common_inefficiencies[ineff_type] = common_inefficiencies.get(ineff_type, 0) + 1
        ...
        if opportunities:
            high_impact_opportunities = [opp for opp in opportunities if opp.potential_impact > 0.15]
            time_sensitive_opportunities = [opp for opp in opportunities if opp.time_sensitivity > 0.7]
        ...
        return recommendations or [{
            ...
            "specific_actions": ["Maintain current strategy", "Monitor for new opportunities"],
