...
class TournamentComplianceValidator:
    ...
    def __init__(self):
        ...
        self.required_reasoning_elements = [
            "analysis", "evidence", "conclusion", "uncertainty"
        ]
    ...
    def validate_reasoning_comment(self, prediction: Prediction) -> ComplianceReport:
        ...
        if prediction.reasoning and len(prediction.reasoning) > self.max_reasoning_length:
            issues.append(ComplianceIssue(
                severity="warning",
                category="format",
        ...
        reasoning_quality_issues = self._validate_reasoning_quality(prediction.reasoning)
        ...
        is_compliant = score >= 0.8 and not any(issue.severity == "error" for issue in issues)
    ...
    def validate_forecast_compliance(self, forecast: Forecast) -> ComplianceReport:
        ...
        is_compliant = (
            ...
            not any(issue.severity == "error" for issue in all_issues)
        )
    ...
    def format_reasoning_for_tournament(self, prediction: Prediction) -> str:
        ...
        for pattern in self.prohibited_patterns:
            formatted_reasoning = re.sub(pattern, "", formatted_reasoning, flags=re.IGNORECASE)

        # Ensure structured format
        if not self._has_structured_format(formatted_reasoning):
            formatted_reasoning = self._add_structure_to_reasoning(formatted_reasoning, prediction)

        # Add transparency footer if needed
        if not self._has_transparency_elements(formatted_reasoning):
            formatted_reasoning = self._add_transparency_elements(formatted_reasoning, prediction)
    ...
    def _validate_reasoning_quality(self, reasoning: str) -> List[ComplianceIssue]:
        ...
        evidence_indicators = ["evidence", "data", "source", "study", "report", "analysis"]
        ...
        uncertainty_indicators = ["uncertain", "confidence", "likely", "probability", "risk", "doubt"]
        if not any(indicator in reasoning.lower() for indicator in uncertainty_indicators):
            issues.append(ComplianceIssue(
                severity="warning",
                category="content",
    ...
    def _validate_transparency_requirements(self, prediction: Prediction) -> List[ComplianceIssue]:
    ...
    def _validate_forecast_level_requirements(self, forecast: Forecast) -> List[ComplianceIssue]:
    ...
    def _calculate_compliance_score(self, issues: List[ComplianceIssue]) -> float:
        ...
        total_penalty = sum(penalty_weights.get(issue.severity, 0.1) for issue in issues)
    ...
    def _has_logical_structure(self, reasoning: str) -> bool:
        ...
        transition_words = [
            "therefore", "however", "because", "since", "given",
            "consequently", "furthermore", "moreover", "in conclusion"
        ]
    ...
    def _has_transparency_elements(self, reasoning: str) -> bool:
        ...
        transparency_elements = [
            "confidence", "uncertainty", "evidence", "method", "approach"
        ]
...
Confidence: {prediction.confidence.value} confidence based on available evidence and analytical approach."""
    ...
    def _add_structure_to_reasoning(self, reasoning: str, prediction: Prediction) -> str:
    ...
    def _add_transparency_elements(self, reasoning: str, prediction: Prediction) -> str:
        ...
        if prediction.method_metadata:
            metadata_str = ", ".join(f"{k}: {v}" for k, v in prediction.method_metadata.items())
