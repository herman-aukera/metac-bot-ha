...
class IngestionService:
    ...
    def parse_questions(self, raw_data: List[Dict[str, Any]]) -> Tuple[List[Question], IngestionStats]:
        """
        ...
        for raw_question in raw_data:
            try:
                ...
                logger.warning(f"Validation error for question {raw_question.get('id', 'unknown')}: {e}")
                self.stats.validation_errors += 1
                self.stats.failed_parsing += 1
                errors.append(str(e))
                
            except ParseError as e:
                logger.warning(f"Parse error for question {raw_question.get('id', 'unknown')}: {e}")
                self.stats.type_conversion_errors += 1
                self.stats.failed_parsing += 1
                errors.append(str(e))
                
            except Exception as e:
                logger.error(f"Unexpected error parsing question {raw_question.get('id', 'unknown')}: {e}")
                self.stats.failed_parsing += 1
                errors.append(str(e))
        ...
        logger.info(f"Ingestion completed: {self.stats.successful_parsed}/{self.stats.total_processed} "
    ...
    def _extract_title(self, data: Dict[str, Any]) -> str:
        ...
        if not title and self.validation_level in [ValidationLevel.STRICT, ValidationLevel.LENIENT]:
    ...
    def _extract_url(self, data: Dict[str, Any]) -> str:
        ...
        if not url.startswith(("http://", "https://")):
            if self.validation_level in [ValidationLevel.STRICT, ValidationLevel.LENIENT]:
    ...
    def _extract_close_time(self, data: Dict[str, Any]) -> datetime:
        ...
        try:
            ...
            if isinstance(close_time_str, datetime):
                return close_time_str.replace(tzinfo=timezone.utc) if close_time_str.tzinfo is None else close_time_str
            ...
            if field in data and data[field]:
                try:
                    ...
                    if isinstance(resolve_time, datetime):
                        return resolve_time.replace(tzinfo=timezone.utc) if resolve_time.tzinfo is None else resolve_time
                    
                    if "T" in str(resolve_time):
                        dt = datetime.fromisoformat(str(resolve_time).replace("Z", "+00:00"))
                        return dt.replace(tzinfo=timezone.utc) if dt.tzinfo is None else dt
        ...
        for field in ["categories", "category", "tags"]:
            if field in data:
                ...
                if isinstance(raw_categories, list):
                    categories.extend([str(cat).strip() for cat in raw_categories if cat])
    ...
    def _extract_metadata(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        metadata_fields = [
            "community_prediction", "num_forecasters", "status", "resolution",
            "is_resolved", "created_time", "created_at", "updated_at"
        ]
    ...
    def _extract_numeric_bounds(self, data: Dict[str, Any], question_type: QuestionType) -> Tuple[Optional[float], Optional[float]]:
        ...
        try:
            ...
            if min_val is not None and max_val is not None and min_val >= max_val:
                if self.validation_level == ValidationLevel.STRICT:
                    raise ValidationError(f"Invalid bounds: min ({min_val}) >= max ({max_val})")
    ...
    def _extract_choices(self, data: Dict[str, Any], question_type: QuestionType) -> Optional[List[str]]:
        """Extract choices for multiple choice questions."""
        if question_type != QuestionType.MULTIPLE_CHOICE:
            return None
        
        choices_raw = data.get("choices") or data.get("options") or data.get("possibilities", {}).get("choices")
        ...
        if isinstance(choices_raw, list):
            ...
            if len(choices) < 2 and self.validation_level in [ValidationLevel.STRICT, ValidationLevel.LENIENT]:
