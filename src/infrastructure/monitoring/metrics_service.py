...
class MetricsCollector:
    ...
    def record_prediction(self, agent_type: str, question_category: str):
        ...
        self.predictions_total.labels(agent_type=agent_type, question_category=question_category).inc()
    ...
    def _evaluate_condition(self, rule: AlertRule) -> bool:
        ...
        try:
            if rule.name == "HighErrorRate":
                ...
                accuracy_history = self.metrics_collector.get_metrics_history('accuracy', 5)
                if accuracy_history:
                ...
                calibration_history = self.metrics_collector.get_metrics_history('calibration_error', 10)
...
class PerformanceTracker:
    ...
    def track_prediction_performance(self, prediction_id: str, accuracy: float,
                                   confidence: float, agent_type: str):
        ...
        class MetricsRequestHandler(MetricsHandler):
            ...
            def do_GET(self):
                if self.path == '/metrics':
                    ...
                    self.end_headers()
                    health_data = json.dumps({'status': 'healthy', 'timestamp': datetime.now().isoformat()})
