...
class SubmissionValidator:
    ...
    def _extract_prediction_data(self, prediction: Prediction) -> Dict[str, Any]:
        ...
        data = {
            ...
            "confidence": prediction.confidence.value if prediction.confidence else None
        }
    ...
    def _validate_required_fields(self, pred_data: Dict[str, Any]) -> List[ValidationError]:
    ...
    def _validate_question_specific(self, question: Question, pred_data: Dict[str, Any]) -> List[ValidationError]:
        """Validate question-specific requirements."""
        errors = []
    ...
    def _validate_prediction_value(self, question: Question, pred_data: Dict[str, Any]) -> List[ValidationError]:
    ...
    def _validate_continuous_prediction(self, question: Question, value: Any) -> List[ValidationError]:
    ...
    def _validate_multiple_choice_prediction(self, question: Question, value: Any) -> List[ValidationError]:
    ...
    def format_prediction_for_submission(
        ...
        if self.tournament_mode:
            formatted = self._apply_tournament_formatting(formatted, question, pred_data)

        logger.info("Prediction formatted for submission",
                   question_type=question.question_type.value,
    ...
    def _apply_tournament_formatting(
        ...
        formatted["tournament_metadata"] = {
            ...
            "tournament_priority": question.metadata.get("tournament_priority", "medium"),
        ...
        formatted["validation_checksum"] = self._calculate_validation_checksum(formatted)

        # Add submission context
        formatted["submission_context"] = {
            "time_until_close": (question.close_time - datetime.now(timezone.utc)).total_seconds() if question.close_time else None,
            "question_age": (datetime.now(timezone.utc) - (question.created_at or datetime.now(timezone.utc))).total_seconds(),
    ...
    def simulate_tournament_conditions(
        ...
        """
        logger.info("Simulating tournament conditions",
                   question_id=question.id,
        ...
        simulation_results = {
            ...
            "prediction_analysis": self._analyze_prediction_for_simulation(question, prediction),
            "tournament_simulation": self._simulate_tournament_dynamics(question, tournament_context),
        ...
        simulation_results["recommendations"] = self._generate_simulation_recommendations(
            question, prediction, simulation_results
        )

        logger.info("Tournament simulation completed",
    ...
    def _analyze_question_for_simulation(self, question: Question) -> Dict[str, Any]:
        ...
        return {
            ...
            "time_until_close": (question.close_time - now).total_seconds() if question.close_time else None,
            "is_urgent": (question.close_time - now).total_seconds() < 86400 if question.close_time else False,
            "is_critical": (question.close_time - now).total_seconds() < 21600 if question.close_time else False,
    ...
    def _analyze_prediction_for_simulation(
        ...
        if question.question_type == QuestionType.BINARY:
            analysis["extremeness"] = min(pred_data.get("prediction_value", 0.5), 1 - pred_data.get("prediction_value", 0.5))
            analysis["is_contrarian"] = self._is_contrarian_prediction(question, pred_data.get("prediction_value"))

        return analysis

    def _is_contrarian_prediction(self, question: Question, prediction_value: float) -> bool:
    ...
    def _simulate_tournament_dynamics(
        ...
        simulation = {
            ...
            "competitive_pressure": "high" if question.metadata.get("tournament_priority") == "critical" else "medium",
            "market_efficiency": self._estimate_market_efficiency(question),
            "scoring_impact": self._estimate_scoring_impact(question, tournament_context),
            "strategic_considerations": self._analyze_strategic_considerations(question, tournament_context)
        }
    ...
    def _estimate_scoring_impact(
        ...
        return {
            ...
            "potential_rank_change": self._estimate_rank_change_potential(question, tournament_context)
        }
    ...
    def _estimate_rank_change_potential(
        ...
        base_change = priority_multiplier.get(question.metadata.get("tournament_priority", "medium"), 1)
    ...
    def _analyze_strategic_considerations(
        ...
        if question.close_time:
            time_until_close = (question.close_time - datetime.now(timezone.utc)).total_seconds()
    ...
    def _calculate_optimal_submission_window(self, question: Question) -> Dict[str, Any]:
    ...
    def _assess_submission_risk(
        ...
        # Contrarian risk
        if self._is_contrarian_prediction(question, pred_data.get("prediction_value", 0.5)):
    ...
    def _generate_simulation_recommendations(
        ...
        strategic_considerations = simulation_results["tournament_simulation"]["strategic_considerations"]
...
class AuditTrailManager:
    ...
    def create_submission_record(
        ...
        logger.info("Created submission record",
                   submission_id=submission_id,
                   question_id=question_id,
                   dry_run=dry_run)
    ...
    def get_submissions_by_question(self, question_id: str) -> List[SubmissionRecord]:
        ...
        return [
            record for record in self.submissions.values()
    ...
    def persist_to_storage(self) -> None:
        ...
        try:
            ...
            logger.info("Audit trail persisted",
    ...
    def load_from_storage(self) -> None:
        ...
        try:
            ...
            logger.info("Audit trail loaded",
                       path=self.storage_path,
                       record_count=len(self.submissions))
    ...
    def generate_audit_report(self) -> Dict[str, Any]:
        ...
        for status in SubmissionStatus:
            status_counts[status.value] = sum(
                1 for record in self.submissions.values()
        ...
        for record in self.submissions.values():
            for error in record.validation_errors:
                validation_error_counts[error.code] = validation_error_counts.get(error.code, 0) + 1
    ...
    def confirm_submission(
        ...
        """
        if submission_id not in self.submissions:
            logger.error("Cannot confirm unknown submission", submission_id=submission_id)
            return
        ...
        logger.info("Submission confirmation recorded",
                   submission_id=submission_id,
                   success=success,
    ...
    def track_submission_attempt(
        ...
        if submission_id not in self.submissions:
            logger.error("Cannot track attempt for unknown submission", submission_id=submission_id)
            return
        ...
        logger.info("Submission attempt tracked",
                   submission_id=submission_id,
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        success_rate = successful_submissions / real_submissions if real_submissions > 0 else 0
    ...
    def _analyze_submission_timing(self) -> Dict[str, Any]:
        ...
        urgent_submissions = [t for t in timing_data if t["time_until_close"] < 21600]  # 6 hours
        normal_submissions = [t for t in timing_data if t["time_until_close"] >= 21600]

        urgent_success_rate = sum(1 for t in urgent_submissions if t["success"]) / len(urgent_submissions) if urgent_submissions else 0
        normal_success_rate = sum(1 for t in normal_submissions if t["success"]) / len(normal_submissions) if normal_submissions else 0
        ...
        return {
            ...
            "timing_recommendation": "avoid_urgent_submissions" if urgent_success_rate < normal_success_rate else "timing_not_critical"
        }
    ...
    def _analyze_category_performance(self) -> Dict[str, Any]:
        ...
        for record in self.submissions.values():
            ...
            if "tournament_metadata" in record.metadata:
                category = record.metadata["tournament_metadata"].get("question_category", "unknown")
            ...
            category_stats[category]["validation_errors"] += len(record.validation_errors)

        # Calculate success rates
        for category, stats in category_stats.items():
            stats["success_rate"] = stats["successful"] / stats["total"] if stats["total"] > 0 else 0
            stats["avg_validation_errors"] = stats["validation_errors"] / stats["total"] if stats["total"] > 0 else 0
    ...
    def _analyze_recent_performance(self, days: int = 7) -> Dict[str, Any]:
        ...
        successful_recent = sum(
            1 for record in recent_submissions
            if record.status == SubmissionStatus.SUBMITTED
        )
        ...
        return {
            ...
            "recent_validation_errors": sum(len(r.validation_errors) for r in recent_submissions),
            "trend": self._calculate_performance_trend(recent_submissions)
        }

    def _calculate_performance_trend(self, recent_submissions: List[SubmissionRecord]) -> str:
        ...
        first_success_rate = sum(1 for r in first_half if r.status == SubmissionStatus.SUBMITTED) / len(first_half)
        second_success_rate = sum(1 for r in second_half if r.status == SubmissionStatus.SUBMITTED) / len(second_half)
    ...
    def _export_json(self, submissions: List[SubmissionRecord], include_metadata: bool) -> str:
    ...
    def _export_csv(self, submissions: List[SubmissionRecord], include_metadata: bool) -> str:
        ...
        # Define CSV columns
        columns = [
            "submission_id", "question_id", "prediction_value", "confidence",
            "timestamp", "status", "dry_run", "validation_error_count"
        ]
        ...
        for record in submissions:
            ...
            if include_metadata:
                ...
                row.update({
                    ...
                    "reasoning_method": tournament_metadata.get("reasoning_method", ""),
                    "tournament_priority": tournament_metadata.get("tournament_priority", "")
                })
    ...
    def _export_summary(self, submissions: List[SubmissionRecord]) -> str:
        ...
        successful = sum(1 for r in submissions if r.status == SubmissionStatus.SUBMITTED)
        ...
        summary_lines = [
            ...
            f"Success Rate: {successful / (total - dry_runs) * 100:.1f}%" if total > dry_runs else "Success Rate: N/A",
            "",
            "Status Distribution:",
        ]
        ...
        # Recent activity
        recent_submissions = [r for r in submissions if r.timestamp >= datetime.now(timezone.utc) - timedelta(days=7)]
...
class DryRunManager:
    ...
    def simulate_submission(
        ...
        simulation_results.update({
            ...
            "competitive_analysis": self._simulate_competitive_impact(question, pred_data, session["tournament_context"]),
            "learning_opportunities": self._identify_learning_opportunities(question, prediction, simulation_results)
        })

        # Update session
        session["submissions"].append(submission_record.submission_id)
        session["simulation_results"][submission_record.submission_id] = simulation_results
        ...
        logger.info("Submission simulation completed",
                   session_id=session_id,
                   submission_id=submission_record.submission_id,
                   validation_result=simulation_results["validation_results"]["result"])
    ...
    def _simulate_api_interaction(
        ...
        formatted_prediction = self.validator.format_prediction_for_submission(question, prediction_data)

        # Simulate API response based on validation
        validation_result, validation_errors = self.validator.validate_prediction(question, prediction_data)
    ...
    def _simulate_competitive_impact(
        ...
        if current_ranking > total_participants * 0.8:
            potential_change = max(potential_change, 1)  # Always positive for low-ranked
        elif current_ranking < total_participants * 0.2:
            potential_change = min(potential_change, 2)  # Limited upside for high-ranked

        new_ranking = max(1, min(total_participants, current_ranking - potential_change))

        return {
            "current_ranking": current_ranking,
            "estimated_new_ranking": new_ranking,
            "potential_change": potential_change,
            "percentile_change": (current_ranking - new_ranking) / total_participants * 100,
            "impact_confidence": min(confidence * 0.8, 0.9),  # Slightly lower than prediction confidence
            "factors": {
                "question_priority": priority,
                "prediction_confidence": confidence,
                "current_position": "low" if current_ranking > total_participants * 0.7 else "high"
            }
        }
    ...
    def end_dry_run_session(self, session_id: str) -> Dict[str, Any]:
        ...
        logger.info("Dry-run session ended",
                   session_id=session_id,
    ...
    def _generate_session_report(self, session: Dict[str, Any]) -> Dict[str, Any]:
        ...
        successful_validations = sum(
            1 for result in simulation_results.values()
        ...
        competitive_impacts = [
            result["competitive_analysis"] for result in simulation_results.values()
        ...
        return {
            "session_summary": {
                ...
                "validation_success_rate": successful_validations / total_submissions if total_submissions > 0 else 0
            },
            "risk_analysis": {
                "high_risk_submissions": high_risk_submissions,
                "risk_rate": high_risk_submissions / total_submissions if total_submissions > 0 else 0,
                "risk_distribution": self._analyze_risk_distribution(simulation_results)
            },
            "learning_analysis": {
                "total_opportunities": len(all_opportunities),
                "opportunity_types": opportunity_types,
                "priority_distribution": self._analyze_priority_distribution(all_opportunities),
                "top_learning_areas": self._identify_top_learning_areas(opportunity_types)
            },
            "competitive_analysis": {
                "analyzed_submissions": len(competitive_impacts),
                "average_ranking_change": avg_ranking_change,
                "potential_ranking_improvement": max([impact["potential_change"] for impact in competitive_impacts], default=0),
                "competitive_readiness": self._assess_competitive_readiness(simulation_results)
            },
            "recommendations": self._generate_session_recommendations(session, simulation_results),
            "detailed_results": simulation_results if len(simulation_results) <= 10 else "truncated_for_brevity"
        }

    def _analyze_risk_distribution(self, simulation_results: Dict[str, Any]) -> Dict[str, int]:
    ...
    def _identify_top_learning_areas(self, opportunity_types: Dict[str, int]) -> List[str]:
        """Identify top learning areas by frequency."""
        sorted_types = sorted(opportunity_types.items(), key=lambda x: x[1], reverse=True)
