...
class TournamentMetaculusClient(MetaculusClient):
    ...
    async def fetch_tournament_questions(
        ...
        """
        logger.info("Fetching tournament questions",
        ...
        for question in questions:
            enhanced_question = await self._enhance_question_with_tournament_data(question)
        ...
        logger.info("Fetched tournament questions",
                   total=len(enhanced_questions),
                   critical=len([q for q in enhanced_questions if self._is_critical_question(q)]))

        return enhanced_questions

    async def _enhance_question_with_tournament_data(self, question: Question) -> Question:
        ...
        tournament_metadata = {
            ...
            "urgency_score": self.deadline_tracker.get(question.id, QuestionDeadlineInfo(
                ...
                submission_window=TimeRange(
                    ...
                    end=question.close_time or datetime.now(timezone.utc) + timedelta(days=30)
                )
            )).urgency_score,
    ...
    def _categorize_question(self, question: Question) -> Optional[QuestionCategory]:
        ...
        # Technology keywords
        tech_keywords = ["ai", "artificial intelligence", "technology", "software", "computer",
                        "algorithm", "machine learning", "blockchain", "cryptocurrency", "agi"]
        if any(keyword in title_lower or keyword in description_lower for keyword in tech_keywords):
            return self.question_categories["technology"]

        # Politics keywords
        politics_keywords = ["election", "president", "congress", "senate", "vote", "political",
                           "government", "policy", "law", "regulation", "biden", "trump"]
        if any(keyword in title_lower or keyword in description_lower for keyword in politics_keywords):
            return self.question_categories["politics"]

        # Economics keywords
        econ_keywords = ["economy", "gdp", "inflation", "market", "stock", "price", "economic",
                        "recession", "growth", "unemployment", "federal reserve", "interest rate"]
        if any(keyword in title_lower or keyword in description_lower for keyword in econ_keywords):
            return self.question_categories["economics"]

        # Science keywords
        science_keywords = ["climate", "covid", "vaccine", "research", "study", "scientific",
                          "medicine", "health", "disease", "temperature", "carbon", "energy"]
        if any(keyword in title_lower or keyword in description_lower for keyword in science_keywords):
    ...
    def _is_critical_question(self, question: Question) -> bool:
        ...
        return self._calculate_question_priority(question) == TournamentPriority.CRITICAL
    ...
    async def get_tournament_context(self, tournament_id: Optional[str] = None) -> Optional[TournamentContext]:
        """
        ...
        try:
            ...
            logger.info("Tournament context retrieved",
    ...
    async def _fetch_tournament_data(self, tournament_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error("Failed to fetch tournament data", tournament_id=tournament_id, error=str(e))
            return None
    ...
    async def optimize_submission_timing(self, question_ids: List[str]) -> Dict[str, Dict[str, Any]]:
        """
        ...
        logger.info("Submission timing optimized",
                   recommendations=len(timing_recommendations))

        return timing_recommendations

    def _calculate_optimal_submission_time(self, deadline_info: QuestionDeadlineInfo) -> Dict[str, Any]:
        ...
        elif deadline_info.submission_window.start <= now <= deadline_info.submission_window.end:
    ...
    async def analyze_competitive_landscape(self, tournament_id: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            analysis = {
                "tournament_context": {
                    ...
                    "time_remaining": context.time_remaining.total_seconds() if context.time_remaining else None
                },
                "question_analysis": {
                    "total_questions": len(questions),
                    "high_priority_questions": len(high_value_questions),
                    "category_distribution": category_distribution,
                    "urgent_questions": len([q for q in questions if self._is_critical_question(q)])
                },
                "strategic_recommendations": self._generate_strategic_recommendations(
                    context, questions, category_distribution
                ),
                "market_inefficiencies": self._identify_market_inefficiencies(questions)
            }
    ...
    def _identify_market_inefficiencies(self, questions: List[Question]) -> List[Dict[str, Any]]:
                ...
                if isinstance(community_pred, (int, float)):
                    if community_pred < 0.1 or community_pred > 0.9:
                        inefficiencies.append({
                            ...
                            "opportunity": "contrarian_position" if community_pred > 0.9 else "confirmation_bias"
                        })

        # Look for questions with low prediction counts
        low_participation = [
            q for q in questions
            if q.metadata.get("prediction_count", 0) < 10
        ]
