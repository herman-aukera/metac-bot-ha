    ...
    async def acquire(self, tokens: int = 1) -> bool:
        ...
        async with self.lock:
            ...
            if self.tokens >= tokens:
                ...
                self.logger.debug("Tokens acquired",
...
class SlidingWindowRateLimiter:
    ...
    async def acquire(self, tokens: int = 1) -> bool:
        ...
        async with self.lock:
            ...
            if len(self.requests) + tokens <= max_requests:
                ...
                self.logger.debug("Requests allowed",
                                tokens_requested=tokens,
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        current_time = time.time()
        cutoff_time = current_time - self.config.window_size
        current_window_requests = len([req for req in self.requests if req > cutoff_time])
...
class RateLimiterManager:
    ...
    def create_rate_limiter(
        ...
        self.logger.info("Created rate limiter",
                        name=name,
...
def create_tournament_rate_limiter(name: str = "tournament") -> SlidingWindowRateLimiter:
