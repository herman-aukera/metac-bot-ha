...
class ReasoningLogger:
    ...
    def __init__(self, base_dir: Optional[Path] = None):
        ...
        logger.info(f"Reasoning logger initialized with base directory: {self.base_dir}")
    ...
    def _generate_markdown_content(
        ...
        if 'research_findings' in reasoning_data:
            lines.extend([
                "## Research Findings",
                "",
                reasoning_data['research_findings'],
                ""
            ])
        ...
        method_specific_keys = [
            'thoughts', 'evaluations', 'actions', 'observations',
            'tree_structure', 'thought_evaluations', 'chain_of_thought',
            'action_sequences', 'self_consistency_checks'
        ]
        ...
        processed_keys = {
            'reasoning', 'reasoning_steps', 'research_findings', 'sources',
            'confidence_analysis', 'key_factors', 'evidence_for', 'evidence_against',
            'uncertainties', 'metadata'
        } | set(method_specific_keys)
        
        remaining_data = {k: v for k, v in reasoning_data.items() if k not in processed_keys}
        if remaining_data:
            lines.extend([
                "## Additional Data",
                ""
            ...
            for key, value in remaining_data.items():
                ...
                lines.extend([
                    f"### {section_title}",
                    "",
                    self._format_data_structure(value),
                    ""
                ])
    ...
    def log_ensemble_reasoning(
        ...
        reasoning_data = {
            ...
            'ensemble_statistics': self._calculate_ensemble_statistics(individual_agents),
            'final_reasoning': ensemble_result.get('reasoning', ''),
            'metadata': {
                'agent_count': len(individual_agents),
                'aggregation_method': aggregation_method,
                'consensus_strength': self._calculate_consensus_strength(individual_agents)
            }
        }
    ...
    def _calculate_ensemble_statistics(self, individual_agents: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Calculate statistics for ensemble reasoning."""
        if not individual_agents:
            return {}
        
        predictions = [agent.get('prediction') for agent in individual_agents if agent.get('prediction') is not None]
        confidences = [agent.get('confidence') for agent in individual_agents if agent.get('confidence') is not None]
    ...
    def _calculate_consensus_strength(self, individual_agents: List[Dict[str, Any]]) -> float:
