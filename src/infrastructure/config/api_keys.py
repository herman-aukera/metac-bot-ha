...
class APIKeyManager:
    ...
    def get_api_key(self, key_name: str) -> Optional[str]:
        ...
        if not value or value.startswith("dummy_"):
            if key_name in self.required_keys:
                ...
                logger.info(
                    f"Optional API key {key_name} not configured, using fallback"
                )
    ...
    def validate_required_keys(self) -> Dict[str, Any]:
        ...
        validation_result = {"valid": True, "missing_keys": [], "warnings": []}
        ...
        for key_name, description in self.required_keys.items():
            ...
            if not value:
                ...
                validation_result["missing_keys"].append(
                    {"key": key_name, "description": description}
                )
    ...
    def log_key_status(self):
        ...
        if validation["valid"]:
            ...
            logger.error(
                f"Missing required API keys: {[k['key'] for k in validation['missing_keys']]}"
            )
