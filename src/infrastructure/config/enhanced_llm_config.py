...
class EnhancedLLMConfig:
    ...
    def __init__(self):
        ...
        if not self.openrouter_key:
            logger.error("OpenRouter API key not found! This is required for tournament operation.")
            raise ValueError("OpenRouter API key is required")

        # Model configuration based on budget status
        self.model_configs = self._setup_model_configs()

        logger.info("Enhanced LLM configuration initialized with task complexity analyzer")
    ...
    def get_llm_for_task(self, task_type: str, question_complexity: str = "medium",
        ...
        if mode_changed:
            logger.warning(f"Operation mode automatically changed: {transition.from_mode.value} → {transition.to_mode.value}")

        # Get model from operation mode manager (integrates complexity analysis)
        recommended_model = self.operation_mode_manager.get_model_for_task(task_type, complexity_assessment)
        ...
        model_config["max_tokens"] = min(model_config.get("max_tokens", 2000),
        ...
        logger.info(f"Using operation mode {self.operation_mode_manager.current_mode.value}: "
        ...
        llm_config = {k: v for k, v in model_config.items()
                     if k not in ["cost_tier"]}
    ...
    def estimate_task_cost(self, prompt: str, task_type: str,
                          question_complexity: str = "medium", complexity_assessment=None) -> Tuple[float, Dict[str, Any]]:
        ...
        if complexity_assessment is not None:
            ...
            if budget_status.status_level == "emergency":
                ...
                model_name = self.model_configs.get(task_type, self.model_configs["simple"])["model"]

            # Estimate tokens
            token_estimate = self.token_tracker.estimate_tokens_for_prompt(prompt, model_name)
    ...
    def can_afford_task(self, prompt: str, task_type: str,
                       question_complexity: str = "medium", complexity_assessment=None) -> Tuple[bool, Dict[str, Any]]:
        """Check if we can afford to execute a task."""
        estimated_cost, details = self.estimate_task_cost(prompt, task_type, question_complexity, complexity_assessment)
    ...
    def record_task_completion(self, question_id: str, prompt: str, response: str,
                              task_type: str, model_used: str, success: bool = True) -> float:
        """Record completion of a task for budget tracking."""
        # Track actual token usage
        actual_usage = self.token_tracker.track_actual_usage(prompt, response, model_used)
    ...
    def assess_question_complexity(self, question_text: str, background_info: str = "",
    ...
    def assess_question_complexity_simple(self, question_text: str, background_info: str = "") -> str:
    ...
    def analyze_question_for_forecasting(self, question_id: str, question_text: str,
                                        background_info: str = "", resolution_criteria: str = "",
                                        fine_print: str = "") -> Dict[str, Any]:
        ...
        # Log the assessment
        self.complexity_analyzer.log_complexity_assessment(question_id, complexity_assessment)
        ...
        return {
            ...
            "total_estimated_cost": research_cost["estimated_cost"] + forecast_cost["estimated_cost"],
            "budget_status": budget_status.status_level,
    ...
    def log_configuration_status(self):
        ...
        logger.info(f"OpenRouter API Key: {'✓ Configured' if self.openrouter_key else '✗ Missing'}")
        ...
        logger.info(f"Task Complexity Analyzer: {'✓ Active' if self.complexity_analyzer else '✗ Missing'}")
        ...
        for complexity in [ComplexityLevel.SIMPLE, ComplexityLevel.MEDIUM, ComplexityLevel.COMPLEX]:
            for task_type in ["research", "forecast"]:
                ...
                model = self.operation_mode_manager.get_model_for_task(task_type, mock_assessment)
