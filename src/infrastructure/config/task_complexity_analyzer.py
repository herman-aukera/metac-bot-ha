...
class TaskComplexityAnalyzer:
    ...
    def _setup_complexity_indicators(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "simple_indicators": {
                "keywords": [
                    "yes/no", "binary", "specific date", "specific number",
                    "announced", "scheduled", "confirmed", "official",
                    "released", "published", "launched", "completed",
                    "will be", "has been", "is scheduled", "is planned"
                ],
                "patterns": [
                    r"\b\d{4}\b",  # Years
            ...
            "medium_indicators": {
                "keywords": [
                    "market", "price", "stock", "election", "vote", "poll",
                    "technology", "development", "research", "study",
                    "approval", "regulation", "law", "policy",
                    "trend", "growth", "decline", "change"
                ],
                "patterns": [
            ...
            "complex_indicators": {
                "keywords": [
                    "geopolitical", "economic", "financial", "systemic",
                    "multiple factors", "complex", "uncertain", "unprecedented",
                    "international", "global", "interdependent", "cascading",
                    "macroeconomic", "geopolitics", "diplomatic", "strategic",
                    "multifaceted", "interconnected", "compound", "aggregate"
                ],
                "patterns": [
    ...
    def assess_question_complexity(self, question_text: str, background_info: str = "",
                                 resolution_criteria: str = "", fine_print: str = "") -> ComplexityAssessment:
    ...
    def _calculate_complexity_scores(self, text: str) -> Dict[str, float]:
        ...
        temporal_count = sum(len(re.findall(pattern, text, re.IGNORECASE))
    ...
    def _determine_complexity_level(self, scores: Dict[str, float]) -> ComplexityLevel:
        ...
        if scores["temporal_score"] >= 1.0 and simple_score >= 2.0 and complex_score <= 1.0:
            return ComplexityLevel.SIMPLE

        # Default to medium for everything else
        return ComplexityLevel.MEDIUM

    def _generate_reasoning(self, scores: Dict[str, float], complexity_level: ComplexityLevel,
                          text: str) -> str:
        """Generate human-readable reasoning for the complexity assessment."""
        reasoning_parts = []

        # Overall assessment
        reasoning_parts.append(f"Assessed as {complexity_level.value.upper()} complexity")

        # Key factors
        if scores["complex_score"] >= 2.0:
            reasoning_parts.append(f"High complexity indicators (score: {scores['complex_score']:.1f})")

        if scores["simple_score"] >= 2.0:
            reasoning_parts.append(f"Simple task indicators (score: {scores['simple_score']:.1f})")

        if scores["uncertainty_score"] >= 1.5:
            reasoning_parts.append(f"High uncertainty language (score: {scores['uncertainty_score']:.1f})")

        if scores["length_score"] >= 2.0:
            reasoning_parts.append(f"Long text requiring detailed analysis (score: {scores['length_score']:.1f})")
    ...
    def get_model_for_task(self, task_type: str, complexity_assessment: ComplexityAssessment,
                          budget_status: str = "normal") -> str:
        ...
        if task_type == "research":
            if complexity_assessment.level == ComplexityLevel.COMPLEX and budget_status == "normal":
        ...
        # For forecast tasks, use complexity-based selection
            ...
            return self.model_recommendations[complexity_assessment.level][budget_status]

        # For other tasks, use simple model
        else:
            return self.model_recommendations[ComplexityLevel.SIMPLE][budget_status]

    def estimate_cost_per_task(self, complexity_assessment: ComplexityAssessment,
                              task_type: str, budget_status: str = "normal") -> Dict[str, Any]:
    ...
    def log_complexity_assessment(self, question_id: str, assessment: ComplexityAssessment):
