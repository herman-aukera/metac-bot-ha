...
from ..prompts.tot_prompts import TOT_SYSTEM_PROMPT, TOT_THOUGHT_GENERATION, TOT_THOUGHT_EVALUATION, TOT_FINAL_SYNTHESIS
...
class TreeOfThoughtAgent(BaseAgent):
    ...
    async def predict(
        ...
        try:
            ...
            prediction = await self._synthesize_prediction(question, thought_tree, research_report)
            ...
            logger.error("Failed to generate ToT prediction", question_id=question.id, error=str(e))
            raise
    ...
    async def _build_thought_tree(
        ...
        current_thoughts = await self._generate_initial_thoughts(question, research_report)
        ...
        for depth in range(1, self.max_depth):
            ...
            promising_thoughts = self._select_promising_thoughts(current_thoughts, evaluations)
            ...
            for thought in promising_thoughts:
                new_thoughts = await self._expand_thought(thought, question, research_report)
...
"""
        ...
        thoughts = self._parse_thoughts(response, depth=parent_thought.depth + 1, parent_id=parent_thought.id)
        return thoughts
        ...
        for line in lines:
            ...
            if line.startswith('EVALUATION'):
                ...
                if current_eval and thought_index <= len(thoughts):
                    evaluations.append(ThoughtEvaluation(
