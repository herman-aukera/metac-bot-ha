...
class TreeOfThoughtAgent(BaseAgent):
    ...
    async def conduct_research(
        ...
        try:
            ...
            component_results = await asyncio.gather(*research_tasks, return_exceptions=True)
            ...
            for i, result in enumerate(component_results):
                if isinstance(result, Exception):
                    self.logger.warning(f"Research failed for component {i}", error=str(result))
                    continue
    ...
    async def generate_prediction(
        ...
        self.logger.info("Starting Tree of Thought prediction", question_id=str(question.id))
        ...
        try:
            ...
            prediction = await self._synthesize_prediction(question, research_report, best_paths)
    ...
    async def _decompose_question(self, question: Question) -> List[str]:
        ...
        response = await self.llm_client.chat_completion(
            messages=[
                {"role": "system", "content": "You are an expert at breaking down complex questions into analyzable components."},
    ...
    async def _research_component(self, component: str, question: Question) -> Tuple[List[Any], str]:
        ...
        try:
            ...
            self.logger.warning(f"Component research failed for {component}", error=str(e))
            return [], f"Research failed for {component}: {str(e)}"

    async def _initialize_reasoning_tree(self, question: Question, research_report: ResearchReport) -> None:
        ...
        for path_type in self.exploration_config.reasoning_path_types:
            initialization_tasks.append(
                self._create_initial_reasoning_path(question, research_report, path_type)
            )

        initial_paths = await asyncio.gather(*initialization_tasks, return_exceptions=True)
        ...
        self.logger.info(f"Initialized reasoning tree with {len(self.reasoning_paths)} paths")
    ...
    async def _create_initial_reasoning_path(
        ...
        initial_step = await self._generate_initial_step(question, research_report, path_type)
    ...
    async def _generate_initial_step(
        ...
        response = await self.llm_client.chat_completion(
            messages=[
                {"role": "system", "content": f"You are an expert using {path_type.value} reasoning approach."},
    ...
    async def _explore_reasoning_tree(self, question: Question, research_report: ResearchReport) -> None:
        ...
        for depth in range(1, self.exploration_config.max_depth):
            ...
            expandable_paths = [
                path for path in self.reasoning_paths.values()
                if not path.is_complete and path.depth < self.exploration_config.max_depth
            ]
            ...
            if len(self.reasoning_paths) > self.exploration_config.max_parallel_paths * 2:
    ...
    async def _evaluate_single_path(self, path: ReasoningPath) -> None:
        ...
        try:
                ...
                if ':' in line:
                    ...
                    try:
                        ...
                        if criterion in [c.name for c in PathEvaluationCriteria]:
                            scores[PathEvaluationCriteria[criterion]] = min(1.0, max(0.0, score))
                    except ValueError:
                        continue
            ...
            # Set default scores
            path.evaluation_scores = {criteria: 0.5 for criteria in PathEvaluationCriteria}

    def _select_paths_for_expansion(self, paths: List[ReasoningPath]) -> List[ReasoningPath]:
        ...
        selected_paths = [path for _, path in scored_paths[:self.exploration_config.path_selection_top_k]]

        self.logger.info(f"Selected {len(selected_paths)} paths for expansion from {len(paths)} candidates")
    ...
    async def _expand_reasoning_path(
        ...
        try:
            ...
            next_step = await self._generate_next_reasoning_step(path, question, research_report)
            path.add_step(next_step)

            # Check if path should be marked as complete
            if path.depth >= self.exploration_config.max_depth or await self._is_path_complete(path):
                path.is_complete = True

        except Exception as e:
            self.logger.warning(f"Failed to expand path {path.id}", error=str(e))
            path.is_complete = True  # Mark as complete to avoid further expansion attempts
    ...
    async def _generate_next_reasoning_step(
        ...
        response = await self.llm_client.chat_completion(
            messages=[
                {"role": "system", "content": f"You are continuing a {path.path_type.value} reasoning analysis."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.5
        )
    ...
    async def _is_path_complete(self, path: ReasoningPath) -> bool:
        ...
        completion_indicators = ["conclusion", "therefore", "in summary", "final assessment"]
        return any(indicator in last_step.content.lower() for indicator in completion_indicators)
    ...
    async def _synthesize_prediction(
        ...
        for i, path in enumerate(best_paths):
            paths_summary.append(f"Path {i+1} ({path.path_type.value}):\n{path.get_reasoning_summary()}")
        ...
        response = await self.llm_client.chat_completion(
            messages=[
                {"role": "system", "content": "You are synthesizing insights from multiple reasoning approaches to make a final prediction."},
        ...
        # Parse response
        probability_value, confidence_value, reasoning, uncertainties = self._parse_synthesis_response(response)
        ...
        return Prediction.create_binary_prediction(
            ...
            confidence=PredictionConfidence.HIGH if confidence_value > 0.7 else
                      PredictionConfidence.MEDIUM if confidence_value > 0.4 else
    ...
    def _create_reasoning_trace(self, question: Question, best_paths: List[ReasoningPath]) -> Optional[ReasoningTrace]:
        ...
        try:
            ...
            return ReasoningTrace.create(
                ...
                overall_confidence=sum(path.get_overall_score() for path in best_paths) / len(best_paths),
                bias_checks=[f"Multiple reasoning path types used: {[p.path_type.value for p in best_paths]}"],
                uncertainty_sources=["Path selection uncertainty", "Synthesis uncertainty"]
            )
