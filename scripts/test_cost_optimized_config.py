...
def test_environment_config():
    ...
    assert os.getenv("DEFAULT_MODEL") == "openai/gpt-5", "DEFAULT_MODEL should be openai/gpt-5"
    assert os.getenv("MINI_MODEL") == "openai/gpt-5-mini", "MINI_MODEL should be openai/gpt-5-mini"
    assert os.getenv("NANO_MODEL") == "openai/gpt-5-nano", "NANO_MODEL should be openai/gpt-5-nano"

    # Test free fallback models
    assert os.getenv("FREE_FORECAST_MODEL") == "moonshotai/kimi-k2:free", "FREE_FORECAST_MODEL should be kimi-k2:free"
    assert os.getenv("FREE_RESEARCH_MODEL") == "openai/gpt-oss-20b:free", "FREE_RESEARCH_MODEL should be gpt-oss-20b:free"

    # Test budget thresholds
    assert float(os.getenv("NORMAL_MODE_THRESHOLD", "0")) == 0.70, "NORMAL_MODE_THRESHOLD should be 0.70"
    assert float(os.getenv("EMERGENCY_MODE_THRESHOLD", "0")) == 0.95, "EMERGENCY_MODE_THRESHOLD should be 0.95"
...
if __name__ == "__main__":
    ...
    logger.info("✓ All tests passed! Cost-optimized configuration is working correctly.")
def test_tri_model_router():
    ...
    try:
        ...
        assert configs["nano"].cost_per_million_input == 0.05, "Nano should cost $0.05/1M tokens"
        assert configs["mini"].cost_per_million_input == 0.25, "Mini should cost $0.25/1M tokens"
        assert configs["full"].cost_per_million_input == 1.50, "Full should cost $1.50/1M tokens"
        ...
        assert "openai/gpt-5-nano" in nano_chain[0], "Nano chain should start with GPT-5 nano"
        assert any("free" in model for model in nano_chain), "Nano chain should include free models"

        # Mini chain should start with GPT-5 mini
        mini_chain = chains["mini"]
        assert "openai/gpt-5-mini" in mini_chain[0], "Mini chain should start with GPT-5 mini"
        assert any("free" in model for model in mini_chain), "Mini chain should include free models"
        ...
        assert any("free" in model for model in full_chain), "Full chain should include free models"

        # Verify no expensive GPT-4o models in primary positions
        all_models = nano_chain + mini_chain + full_chain
        expensive_models = [m for m in all_models if "gpt-4o" in m and not m.startswith("metaculus/")]
        assert len(expensive_models) == 0, f"Found expensive GPT-4o models: {expensive_models}"
...
def test_anti_slop_prompts():
    ...
    try:
        ...
        assert "ANTI-SLOP" in research_prompt, "Research prompt should include anti-slop directives"
        assert "Cite every factual claim" in research_prompt, "Research prompt should require citations"
        ...
        assert "calibrate predictions" in binary_prompt, "Binary prompt should include calibration"
        assert "Probability: XX%" in binary_prompt, "Binary prompt should specify probability format"
...
async def test_cost_estimation():
    ...
    try:
        ...
        normal_cost = tri_model_router.get_cost_estimate("forecast", 1000, "high", 50.0)  # 50% budget remaining
        assert normal_cost > 0, "Normal mode should have some cost"

        # Test emergency mode (should use free models)
        emergency_cost = tri_model_router.get_cost_estimate("forecast", 1000, "high", 5.0)  # 5% budget remaining
...
if __name__ == "__main__":
    ...
    logger.info("🎉 All tests passed! Cost-optimized GPT-5 → Free model configuration is working correctly!")
    logger.info("💰 Expected cost savings: ~95% reduction vs GPT-4o fallbacks")
    logger.info("📊 Expected capacity: ~5000+ questions vs ~200-300 with expensive fallbacks")
