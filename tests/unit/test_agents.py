...
class TestChainOfThoughtAgent:
    ...
    async def test_cot_agent_forecast(self, cot_agent, sample_question, mock_search_results):
        ...
        assert final_prediction.confidence in [PredictionConfidence.HIGH, PredictionConfidence.MEDIUM]
        ...
        assert cot_agent.llm_client.chat_completion.call_count == 4  # 4 calls: breakdown, research areas, synthesis, prediction
    ...
    async def test_cot_agent_error_handling(self, cot_agent, sample_question):
        ...
        mock_synthesis_response = {
            "choices": [{"message": {"content": '''{"executive_summary": "Limited analysis due to search issues", "detailed_analysis": "Fallback analysis", "key_factors": [], "base_rates": {}, "confidence_level": 0.5, "reasoning_steps": [], "evidence_for": [], "evidence_against": [], "uncertainties": []}'''}}]
        }
        mock_prediction_response = {
            "choices": [{"message": {"content": '''{"probability": 0.42, "confidence": "medium", "reasoning": "Fallback prediction", "reasoning_steps": ["Fallback step"]}'''}}]
        }
...
REASONING: Based on expert analysis and progress metrics, the likelihood is moderate."""
        ...
        assert isinstance(forecast, Forecast)
        assert forecast.question_id == sample_question.id
        assert forecast.final_prediction.result.binary_probability == 0.42  # Agent returns 0.42 based on mock
        assert forecast.final_prediction.confidence == PredictionConfidence.MEDIUM  # Agent actually returns MEDIUM
...
REASONING: Expert surveys and progress metrics suggest moderate likelihood with significant uncertainty.""",
        ...
        assert isinstance(forecast, Forecast)
        assert forecast.question_id == sample_question.id
        assert forecast.final_prediction.result.binary_probability == 0.38  # Agent returns 0.38 based on mock
        assert forecast.final_prediction.confidence == PredictionConfidence.MEDIUM  # Agent actually returns MEDIUM
...
class TestEnsembleAgent:
    ...
    def _create_test_prediction(self, question_id: UUID, probability: float, confidence: PredictionConfidence, method: PredictionMethod) -> Prediction:
    ...
    def _create_test_forecast(self, question_id: UUID, probability: float, confidence: PredictionConfidence, method: PredictionMethod) -> Forecast:
        """Helper to create a test forecast."""
        research_report = self._create_test_research_report(question_id)
        prediction = self._create_test_prediction(question_id, probability, confidence, method)
    ...
    async def test_ensemble_weighted_average(self, ensemble_agent, sample_question, mock_agents):
        ...
        predictions = [
            self._create_test_prediction(sample_question.id, 0.3, PredictionConfidence.HIGH, PredictionMethod.CHAIN_OF_THOUGHT),
            self._create_test_prediction(sample_question.id, 0.5, PredictionConfidence.VERY_HIGH, PredictionMethod.TREE_OF_THOUGHT),
    ...
    async def test_ensemble_simple_average(self, mock_settings):
        ...
        predictions = [
            self._create_test_prediction(sample_question.id, 0.4, PredictionConfidence.HIGH, PredictionMethod.CHAIN_OF_THOUGHT),
            self._create_test_prediction(sample_question.id, 0.6, PredictionConfidence.HIGH, PredictionMethod.TREE_OF_THOUGHT)
        ]
