...
def mock_search_client():
    ...
    client.search = AsyncMock(return_value=[
        {"url": "http://example.com/1", "title": "Test Result 1", "snippet": "Test snippet 1"},
        {"url": "http://example.com/2", "title": "Test Result 2", "snippet": "Test snippet 2"}
    ])
    return client
...
class TestTreeExplorationConfig:
    ...
    def test_custom_config(self):
        ...
        config = TreeExplorationConfig(
            ...
            reasoning_path_types=[ReasoningPathType.ANALYTICAL, ReasoningPathType.CAUSAL]
        )
...
class TestTreeOfThoughtAgent:
    ...
    @pytest.mark.asyncio
    async def test_decompose_question(self, tree_agent, sample_question, mock_llm_client):
    ...
    @pytest.mark.asyncio
    async def test_research_component(self, tree_agent, sample_question, mock_search_client):
        """Test researching a specific component."""
        component = "Technical feasibility of AGI"

        sources, summary = await tree_agent._research_component(component, sample_question)
    ...
    @pytest.mark.asyncio
    async def test_conduct_research(self, tree_agent, sample_question, mock_llm_client, mock_search_client):
    ...
    @pytest.mark.asyncio
    async def test_initialize_reasoning_tree(self, tree_agent, sample_question, sample_research_report, mock_llm_client):
        """Test reasoning tree initialization."""
        # Mock initial step generation
        mock_llm_client.chat_completion.return_value = "Initial analytical reasoning step"

        await tree_agent._initialize_reasoning_tree(sample_question, sample_research_report)

        # Should create paths for each configured reasoning type
        assert len(tree_agent.reasoning_paths) == len(tree_agent.exploration_config.reasoning_path_types)
    ...
    @pytest.mark.asyncio
    async def test_create_initial_reasoning_path(self, tree_agent, sample_question, sample_research_report, mock_llm_client):
        """Test creating initial reasoning path."""
        mock_llm_client.chat_completion.return_value = "Empirical analysis of AGI development"
    ...
    @pytest.mark.asyncio
    async def test_generate_initial_step(self, tree_agent, sample_question, sample_research_report, mock_llm_client):
        """Test generating initial reasoning step."""
        mock_llm_client.chat_completion.return_value = "Analytical approach to AGI timeline"
    ...
    @pytest.mark.asyncio
    async def test_generate_next_reasoning_step(self, tree_agent, sample_question, sample_research_report, mock_llm_client):
        """Test generating next reasoning step."""
        # Create path with existing step
        path = ReasoningPath(
            path_type=ReasoningPathType.CAUSAL,
            sub_components=["Factor 1", "Factor 2"]
        )
        ...
        mock_llm_client.chat_completion.return_value = "Deeper causal analysis building on previous step"

        next_step = await tree_agent._generate_next_reasoning_step(path, sample_question, sample_research_report)
    ...
    def test_parse_synthesis_response(self, tree_agent):
        ...
        prob, conf, reasoning, uncertainties = tree_agent._parse_synthesis_response(response)
    ...
    @pytest.mark.asyncio
    async def test_generate_prediction_integration(self, tree_agent, sample_question, sample_research_report, mock_llm_client):
...
UNCERTAINTIES: Technical challenges and timeline uncertainty"""
            elif 'LOGICAL_COHERENCE' in content or 'Evaluate this reasoning path' in content:
                return """
                LOGICAL_COHERENCE: 0.8
                EVIDENCE_STRENGTH: 0.7
                NOVELTY: 0.6
        ...
        prediction = await tree_agent.generate_prediction(sample_question, sample_research_report)
    ...
    async def test_prune_reasoning_paths(self, tree_agent):
        ...
        assert len(tree_agent.reasoning_paths) == tree_agent.exploration_config.max_parallel_paths

        # Should keep the highest scoring paths
        remaining_scores = [path.get_overall_score() for path in tree_agent.reasoning_paths.values()]
