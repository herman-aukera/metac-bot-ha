...
from src.agents.react_agent import ReActAgent, ActionType, ActionValidationResult, ActionContext, ReActStep
...
def mock_search_client():
    ...
    client.search = AsyncMock(return_value=[
        {"title": "Test Result", "snippet": "Test snippet", "url": "http://test.com"}
    ])
    return client
...
class TestEnhancedReActAgent:
    ...
    def test_is_action_contextually_appropriate(self, react_agent, sample_question):
        ...
        assert react_agent._is_action_contextually_appropriate(
    ...
    def test_action_needs_refinement(self, react_agent):
        ...
        assert react_agent._action_needs_refinement(
        ...
        assert react_agent._action_needs_refinement(
            ActionType.SEARCH, "artificial general intelligence timeline research", ActionContext()
        ) is False
    ...
    async def test_adapt_reasoning_strategy(self, react_agent, sample_question):
        ...
        steps = [
            ReActStep(1, "thought", ActionType.SEARCH, "input", "obs", "reasoning", confidence_change=0.1),
            ReActStep(2, "thought", ActionType.ANALYZE, "input", "obs", "reasoning", confidence_change=0.05),
            ReActStep(3, "thought", ActionType.THINK, "input", "obs", "reasoning", confidence_change=0.02)
        ]

        adapted_context = await react_agent._adapt_reasoning_strategy(context, steps, sample_question)
    ...
    def test_is_stuck_in_loop(self, react_agent):
        ...
        diverse_steps = [
            ...
            ReActStep(3, "thought3", ActionType.SYNTHESIZE, "input", "obs", "reasoning"),
        ...
        repetitive_steps = [
            ReActStep(1, "similar thought about AI", ActionType.THINK, "input", "obs", "reasoning"),
            ReActStep(2, "similar thought about AI", ActionType.THINK, "input", "obs", "reasoning"),
            ReActStep(3, "similar thought about AI", ActionType.THINK, "input", "obs", "reasoning"),
            ReActStep(4, "similar thought about AI", ActionType.THINK, "input", "obs", "reasoning")
        ]
    ...
    async def test_create_reasoning_trace(self, react_agent, sample_question):
        ...
        steps = [
            ReActStep(1, "thought1", ActionType.SEARCH, "search query", "search results", "reasoning1"),
            ReActStep(2, "thought2", ActionType.ANALYZE, "analysis target", "analysis results", "reasoning2"),
            ReActStep(3, "thought3", ActionType.BIAS_CHECK, "bias check", "bias results", "reasoning3")
        ]
    ...
    async def test_execute_new_action_types(self, react_agent, sample_question):
        ...
        result = await react_agent._execute_validate_action("test validation", sample_question)
        assert "Validation of 'test validation'" in result

        # Test bias check action
        result = await react_agent._execute_bias_check_action("test bias check", sample_question)
        assert "Bias check for 'test bias check'" in result

        # Test uncertainty assessment action
        result = await react_agent._execute_uncertainty_assess_action("test uncertainty", sample_question)
    ...
    def test_calculate_confidence_change(self, react_agent):
        ...
        change = react_agent._calculate_confidence_change(
            ActionType.SEARCH, "Found strong evidence and research data", "This supports the hypothesis", context
        )
        assert change > 0

        # Negative change for poor results
        change = react_agent._calculate_confidence_change(
            ActionType.SEARCH, "No results found, unclear information", "This is uncertain and conflicting", context
        )
        assert change < 0

    def test_suggest_alternative_action(self, react_agent):
        """Test alternative action suggestion."""
        context = ActionContext(previous_actions=[ActionType.THINK, ActionType.THINK, ActionType.SEARCH])
        steps = []
        ...
        assert alternative in [ActionType.ANALYZE, ActionType.SYNTHESIZE, ActionType.VALIDATE]
    ...
    @pytest.mark.asyncio
    async def test_full_prediction_cycle(self, react_agent, sample_question, mock_llm_client):
        ...
        ] * 3)  # Multiply to ensure we have enough responses
