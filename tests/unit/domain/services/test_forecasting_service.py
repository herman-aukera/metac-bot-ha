...
class TestForecastingService:
    ...
    @pytest.fixture
    def binary_prediction_high_conf(self, sample_question_id, sample_research_report_id):
    ...
    @pytest.fixture
    def binary_prediction_medium_conf(self, sample_question_id, sample_research_report_id):
    ...
    def test_init_creates_service_with_supported_methods(self, forecasting_service):
        ...
        assert PredictionMethod.CHAIN_OF_THOUGHT in forecasting_service.supported_methods
        ...
        assert PredictionMethod.SELF_CONSISTENCY in forecasting_service.supported_methods
    ...
    def test_aggregate_predictions_weighted_average(
        ...
        result = forecasting_service.aggregate_predictions(predictions, method="weighted_average")
    ...
    def test_aggregate_predictions_median(
        ...
        predictions = [binary_prediction_high_conf, binary_prediction_medium_conf, binary_prediction_low_conf]
    ...
    def test_aggregate_predictions_confidence_weighted(
        ...
        result = forecasting_service.aggregate_predictions(predictions, method="confidence_weighted")
        ...
        medium_weight = binary_prediction_medium_conf.get_confidence_score() ** 2  # 0.6^2
        total_weight = high_weight + medium_weight
        expected_prob = (0.7 * high_weight + 0.6 * medium_weight) / total_weight
    ...
    def test_aggregate_predictions_unsupported_method_raises_error(
        ...
        with pytest.raises(ValueError, match="Unsupported aggregation method: invalid_method"):
            forecasting_service.aggregate_predictions(predictions, method="invalid_method")
    ...
    def test_confidence_weighted_average_empty_list_raises_error(self, forecasting_service):
        """Test confidence weighted average with empty list raises error."""
        with pytest.raises(ValueError, match="Cannot calculate average of empty prediction list"):
    ...
    def test_confidence_weighted_average_single_prediction(
        ...
        result = forecasting_service.confidence_weighted_average([binary_prediction_high_conf])
    ...
    def test_confidence_weighted_average_zero_total_weight(
        ...
        with patch.object(pred1, 'get_confidence_score', return_value=0.0), \
    ...
    def test_score_to_confidence_mapping(self, forecasting_service):
        ...
        assert forecasting_service._score_to_confidence(0.1) == PredictionConfidence.VERY_LOW
        assert forecasting_service._score_to_confidence(0.3) == PredictionConfidence.LOW
        assert forecasting_service._score_to_confidence(0.5) == PredictionConfidence.MEDIUM
        assert forecasting_service._score_to_confidence(0.7) == PredictionConfidence.HIGH
        assert forecasting_service._score_to_confidence(0.9) == PredictionConfidence.VERY_HIGH
        
        # Boundary conditions
        assert forecasting_service._score_to_confidence(0.2) == PredictionConfidence.VERY_LOW
        assert forecasting_service._score_to_confidence(0.4) == PredictionConfidence.LOW
        assert forecasting_service._score_to_confidence(0.6) == PredictionConfidence.MEDIUM
        assert forecasting_service._score_to_confidence(0.8) == PredictionConfidence.HIGH
    ...
    def test_validate_forecast_quality_high_variance_warning(
        ...
        with patch.object(sample_forecast, 'calculate_prediction_variance', return_value=0.15):
            quality = forecasting_service.validate_forecast_quality(sample_forecast)
            
            assert any("High prediction variance" in warning for warning in quality["warnings"])
    ...
    def test_validate_forecast_quality_low_research_quality_warning(
        ...
        with patch.object(sample_forecast, 'calculate_prediction_variance', return_value=0.05):
            quality = forecasting_service.validate_forecast_quality(sample_forecast)
            
            assert any("Low average research quality" in warning for warning in quality["warnings"])
    ...
    def test_validate_forecast_quality_high_quality_forecast(
        ...
        with patch.object(sample_forecast, 'calculate_prediction_variance', return_value=0.03):  # High consensus
            quality = forecasting_service.validate_forecast_quality(sample_forecast)
    ...
    def test_validate_forecast_quality_medium_quality_forecast(
        ...
        with patch.object(sample_forecast, 'calculate_prediction_variance', return_value=0.08):  # Medium consensus
            quality = forecasting_service.validate_forecast_quality(sample_forecast)
    ...
    def test_aggregate_predictions_only_binary_supported_error(
        ...
        with pytest.raises(NotImplementedError, match="Only binary predictions supported currently"):
            forecasting_service.aggregate_predictions([numeric_pred], method="weighted_average")
    ...
    def test_confidence_weighted_aggregation_falls_back_to_weighted_average(
        ...
        with patch.object(pred1, 'get_confidence_score', return_value=0.0), \
            ...
            with patch.object(forecasting_service, '_weighted_average_aggregation') as mock_weighted:
                mock_result = Mock(spec=Prediction)
                mock_weighted.return_value = mock_result
                
                result = forecasting_service.aggregate_predictions([pred1, pred2], method="confidence_weighted")
                
                mock_weighted.assert_called_once_with([pred1, pred2], sample_question_id)
