...
from src.infrastructure.config.settings import Settings, MetaculusConfig
...
class TestMetaculusClientComprehensive:
    ...
    @pytest.mark.asyncio
    async def test_fetch_questions_success(self, metaculus_client, sample_question_data):
        """Test successful question fetching."""
        mock_response_data = {
            "results": [sample_question_data, {**sample_question_data, "id": 12346, "title": "Another question"}]
        }
    ...
    @pytest.mark.asyncio
    async def test_fetch_questions_with_categories(self, metaculus_client, sample_question_data):
    ...
    @pytest.mark.asyncio
    async def test_submit_prediction_old_interface(self, metaculus_client, sample_prediction):
        ...
        with patch('httpx.AsyncClient.post') as mock_post:
            ...
            result = await metaculus_client.submit_prediction(12345, sample_prediction, "Custom comment")
    ...
    @pytest.mark.asyncio
    async def test_submit_prediction_not_authenticated(self, metaculus_client, sample_prediction):
    ...
    @pytest.mark.asyncio
    async def test_submit_prediction_http_error(self, metaculus_client, sample_prediction):
    ...
    @pytest.mark.asyncio
    async def test_submit_prediction_exception(self, metaculus_client, sample_prediction):
    ...
    @pytest.mark.asyncio
    async def test_submit_prediction_old_interface_missing_prediction(self, metaculus_client):
        """Test old interface with missing prediction parameter."""
        with pytest.raises(ValueError, match="prediction is required when using old interface"):
    ...
    async def test_submit_prediction_invalid_prediction_type(self, metaculus_client):
        ...
        with pytest.raises(ValueError, match="Prediction must have either binary_probability or numeric_value"):
    ...
    def test_parse_question_multiple_choice(self, metaculus_client, sample_question_data):
    ...
    @pytest.mark.asyncio
    async def test_fetch_benchmark_questions(self, metaculus_client, sample_question_data):
        """Test fetching benchmark questions."""
        # Mock resolved question
        resolved_question_data = {**sample_question_data, "status": "resolved", "resolution": "yes"}
    ...
    @pytest.mark.asyncio
    async def test_batch_fetch_questions_success(self, metaculus_client, sample_question_data):
        ...
        with patch.object(metaculus_client, 'fetch_question', side_effect=mock_fetch_question):
    ...
    async def test_get_question_alias(self, metaculus_client):
        ...
        with patch.object(metaculus_client, 'fetch_question', return_value=Mock()) as mock_fetch:
    ...
    async def test_get_questions_alias(self, metaculus_client):
        ...
        with patch.object(metaculus_client, 'fetch_questions', return_value=[]) as mock_fetch:
