...
from src.application.dispatcher import (
    Dispatcher, DispatcherConfig, DispatcherStats, DispatcherError
)
from src.application.ingestion_service import ValidationLevel, IngestionStats
...
def api_config():
    ...
    return APIConfig(
        base_url="https://test.metaculus.com"
    )
...
class TestDispatcher:
    ...
    def test_init_default_config(self):
        ...
        with patch('src.application.dispatcher.MetaculusAPI'), \
    ...
    @patch('src.application.dispatcher.IngestionService')
    @patch('src.application.dispatcher.ForecastService')
    def test_run_success(self, mock_forecast_service_cls, mock_ingestion_service_cls, 
                        mock_api_cls, sample_raw_questions, sample_questions, sample_forecasts):
        ...
        mock_ingestion_stats = IngestionStats(
            successful_parsed=len(sample_questions),
            failed_parsing=0
        )
        mock_ingestion_service.parse_questions.return_value = (sample_questions, mock_ingestion_stats)
        ...
        mock_ingestion_service.parse_questions.assert_called_once_with(sample_raw_questions)
        assert mock_forecast_service.generate_forecast.call_count == 2
    
    @patch('src.application.dispatcher.MetaculusAPI')
    @patch('src.application.dispatcher.IngestionService')
    @patch('src.application.dispatcher.ForecastService')
    def test_run_api_error(self, mock_forecast_service_cls, mock_ingestion_service_cls, mock_api_cls):
    ...
    @patch('src.application.dispatcher.IngestionService')
    @patch('src.application.dispatcher.ForecastService')
    def test_run_dry_run_mode(self, mock_forecast_service_cls, mock_ingestion_service_cls, 
                             mock_api_cls, sample_raw_questions, sample_questions):
        ...
        mock_ingestion_service = Mock()
        mock_ingestion_stats = IngestionStats(successful_parsed=len(sample_questions), failed_parsing=0)
        mock_ingestion_service.parse_questions.return_value = (sample_questions, mock_ingestion_stats)
        mock_ingestion_service_cls.return_value = mock_ingestion_service
    ...
    @patch('src.application.dispatcher.IngestionService')
    @patch('src.application.dispatcher.ForecastService')
    def test_run_forecast_failures(self, mock_forecast_service_cls, mock_ingestion_service_cls, 
                                  mock_api_cls, sample_raw_questions, sample_questions):
        ...
        mock_forecast_service.generate_forecast.side_effect = [mock_forecast, Exception("Forecast error")]
        mock_forecast_service_cls.return_value = mock_forecast_service
    ...
    @patch('src.application.dispatcher.IngestionService')
    @patch('src.application.dispatcher.ForecastService')
    def test_run_batch(self, mock_forecast_service_cls, mock_ingestion_service_cls, 
                      mock_api_cls, sample_raw_questions, sample_questions, sample_forecasts):
        ...
        mock_forecast_service = Mock()
        mock_forecast_service.generate_forecast.side_effect = sample_forecasts * 2  # For multiple batches
        mock_forecast_service_cls.return_value = mock_forecast_service
