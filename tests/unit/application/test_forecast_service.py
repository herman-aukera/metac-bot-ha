...
class TestForecastService:
    ...
    def test_validate_forecast_success(self, forecast_service, sample_binary_question):
        ...
        forecast_service.validate_forecast(
            sample_binary_question, probability, confidence, reasoning
        )

    def test_validate_forecast_closed_question(
        self, forecast_service, sample_closed_question
    ):
        ...
        with pytest.raises(ForecastValidationError, match="Question is closed"):
            forecast_service.validate_forecast(
                sample_closed_question, probability, confidence, reasoning
            )

    def test_validate_forecast_non_binary_question(
        self, forecast_service, sample_numeric_question
    ):
        ...
        with pytest.raises(
            ForecastValidationError, match="Only binary questions are supported"
        ):
            forecast_service.validate_forecast(
                sample_numeric_question, probability, confidence, reasoning
            )

    def test_validate_forecast_empty_reasoning(
        self, forecast_service, sample_binary_question
    ):
    ...
    def test_validate_forecast_extreme_probability(
        self, forecast_service, sample_binary_question
    ):
        ...
        with pytest.raises(ForecastValidationError, match="Extreme probability values"):
            forecast_service.validate_forecast(
                sample_binary_question, probability_low, confidence, reasoning
            )

        with pytest.raises(ForecastValidationError, match="Extreme probability values"):
            forecast_service.validate_forecast(
                sample_binary_question, probability_high, confidence, reasoning
            )
    ...
    def test_create_forecast_validation_error(
        self, forecast_service, sample_closed_question
    ):
    ...
    def test_generate_forecast_with_community_prediction(
        self, forecast_service, sample_binary_question
    ):
    ...
    def test_generate_forecast_closed_question(
        self, forecast_service, sample_closed_question
    ):
        """Test forecast generation with closed question."""
        with pytest.raises(
            ForecastValidationError,
            match="Cannot generate forecast for closed question",
        ):
            forecast_service.generate_forecast(sample_closed_question)

    def test_generate_forecast_non_binary_question(
        self, forecast_service, sample_numeric_question
    ):
        ...
        assert (
            sample_numeric_question.min_value
            <= forecast.final_prediction.result.numeric_value
            <= sample_numeric_question.max_value
        )
        assert len(forecast.research_reports) > 0

    # Test score_forecast method
    def test_score_forecast_unresolved_question(
        self, forecast_service, sample_forecast, sample_binary_question
    ):
    ...
    def test_batch_score_forecasts(
        self, forecast_service, sample_binary_question, sample_forecast
    ):
    ...
    def test_batch_score_forecasts_missing_questions(
        self, forecast_service, sample_forecast
    ):
    ...
    def test_create_mock_research_report(
        self, forecast_service, sample_binary_question
    ):
...
class TestForecastServiceIntegration:
    ...
    def test_forecast_consistency(self, forecast_service, sample_binary_question):
        ...
        forecasts = [
            forecast_service.generate_forecast(sample_binary_question) for _ in range(5)
        ]

        # Extract final probabilities
        probabilities = [
            f.final_prediction.result.binary_probability for f in forecasts
        ]
