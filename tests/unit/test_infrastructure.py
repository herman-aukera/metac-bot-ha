...
class TestLLMClient:
    ...
    async def test_openai_request_retry_on_failure(self, llm_client):
        ...
        with patch("httpx.AsyncClient.post") as mock_post:
            ...
            mock_failed_response.raise_for_status.side_effect = Exception(
                "Server Error"
            )
    ...
    async def test_wikipedia_search(self, search_client):
        ...
        with patch("httpx.AsyncClient.get") as mock_get:
            ...
            mock_search_response.json.return_value = {
                "pages": [
                    {
                        "key": "Artificial_general_intelligence",
    ...
    async def test_search_deduplication(self, search_client):
        ...
        with patch("httpx.AsyncClient.get") as mock_get:
            ...
            mock_wiki_search_response.json.return_value = {
                "pages": [{"key": "AI_Progress", "title": "AI Progress Report"}]
            }
...
class TestMetaculusClient:
    ...
    async def test_get_question_success(
        self, metaculus_client, mock_metaculus_response
    ):
