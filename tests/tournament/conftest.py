...
def mock_tournament_forecast_service():
    ...
    async def mock_generate_forecast(question, agent_types=None, timeout=60, **kwargs):
        ...
        return Forecast(
            ...
            method="ensemble" if agent_types and len(agent_types) > 1 else "single_agent",
...
def mock_tournament_ensemble_service():
    ...
    def mock_aggregate_forecasts(forecasts, method="weighted_average"):
        ...
        if method == "weighted_average":
            ...
            if total_weight > 0:
                avg_prediction = sum(p * w for p, w in zip(predictions, weights)) / total_weight
            else:
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "tournament: Tournament simulation tests"
    )
    config.addinivalue_line(
        "markers", "pressure: Competitive pressure tests"
    )
    config.addinivalue_line(
        "markers", "resilience: Recovery and resilience tests"
    )
    config.addinivalue_line(
        "markers", "performance: Agent performance tests"
    )
    config.addinivalue_line(
        "markers", "calibration: Calibration validation tests"
    )
    config.addinivalue_line(
        "markers", "bias: Bias detection tests"
    )
