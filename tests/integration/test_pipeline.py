...
class TestForecastingPipelineIntegration:
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_forecasting_single_agent(self, pipeline, sample_question_data, mock_clients, mock_search_results):
        ...
        metaculus_client.submit_prediction.return_value = {"status": "dry_run", "would_submit": True}
    ...
    async def test_end_to_end_forecasting_ensemble(self, pipeline, sample_question_data, mock_clients, mock_search_results):
        ...
        assert 0.3 <= ensemble_prediction <= 0.5  # Should be within range of individual predictions
    ...
    @pytest.mark.asyncio
    async def test_pipeline_partial_failure_handling(self, pipeline, mock_clients, sample_question_data):
    ...
    @pytest.mark.asyncio
    async def test_pipeline_caching(self, pipeline, mock_clients, sample_question_data, mock_search_results):
    ...
    @pytest.mark.asyncio
    async def test_pipeline_metrics_collection(self, pipeline, mock_clients, sample_question_data, mock_search_results):
