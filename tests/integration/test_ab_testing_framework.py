...
class ABTestingFramework:
    ...
    def run_ab_test(self, questions: List[Question], prompt_variants: List[str],
                   sample_size: int = 100) -> Dict[str, Any]:
        """Run A/B test comparing prompt variants."""
        results = {variant: {"responses": [], "token_usage": [], "performance_metrics": {}}
        ...
        for i in range(sample_size):
            ...
            if variant == "basic_calibrated":
                prompt = self.prompts.generate_basic_calibrated_prompt(question, "test research")
            elif variant == "scenario_analysis":
                prompt = self.prompts.generate_scenario_analysis_prompt(question, "test research")
            elif variant == "overconfidence_reduction":
                prompt = self.prompts.generate_overconfidence_reduction_prompt(question, "test research")
        ...
        for variant in prompt_variants:
            results[variant]["performance_metrics"] = self._calculate_performance_metrics(
                results[variant]
            )
    ...
    def _simulate_response(self, prompt: str, variant: str) -> Dict[str, Any]:
        ...
        return {
            "quality_score": min(1.0, base_quality + quality_boost + random.uniform(-0.1, 0.1)),
            "calibration_score": random.uniform(0.6, 0.9),
            "token_efficiency": 1.0 / token_cost_multiplier,
            "response_time": random.uniform(2.0, 8.0)
        }

    def _calculate_performance_metrics(self, variant_results: Dict[str, List]) -> Dict[str, float]:
        ...
        return {
            ...
            "avg_calibration_score": sum(r["calibration_score"] for r in responses) / len(responses),
            "avg_token_efficiency": sum(r["token_efficiency"] for r in responses) / len(responses),
            "avg_token_usage": sum(token_usage) / len(token_usage),
            "total_cost_estimate": sum(token_usage) * 0.0006 / 1000,  # GPT-4o-mini pricing
    ...
    def _calculate_consistency(self, responses: List[Dict[str, Any]]) -> float:
        ...
        variance = sum((x - sum(quality_scores)/len(quality_scores))**2 for x in quality_scores) / len(quality_scores)
...
class TestABTestingFramework:
    ...
    def test_ab_test_execution(self):
        ...
        prompt_variants = ["basic_calibrated", "scenario_analysis", "overconfidence_reduction"]
    ...
    def test_performance_metrics_calculation(self):
        ...
        for variant in prompt_variants:
            ...
            expected_metrics = [
                "avg_quality_score", "avg_calibration_score", "avg_token_efficiency",
                "avg_token_usage", "total_cost_estimate", "response_consistency"
            ]
    ...
    def test_variant_comparison(self):
        ...
        basic_tokens = results["basic_calibrated"]["performance_metrics"]["avg_token_usage"]
        scenario_tokens = results["scenario_analysis"]["performance_metrics"]["avg_token_usage"]

        # Scenario analysis should use more tokens (more comprehensive)
        assert scenario_tokens > basic_tokens, "Scenario analysis should use more tokens"

        # Compare quality scores
        basic_quality = results["basic_calibrated"]["performance_metrics"]["avg_quality_score"]
        scenario_quality = results["scenario_analysis"]["performance_metrics"]["avg_quality_score"]

        # Quality differences should be measurable
        assert abs(basic_quality - scenario_quality) >= 0, "Should measure quality differences"
    ...
    def test_cost_efficiency_analysis(self):
        ...
        basic_efficiency = (results["basic_calibrated"]["performance_metrics"]["avg_quality_score"] /
                           results["basic_calibrated"]["performance_metrics"]["total_cost_estimate"])
        scenario_efficiency = (results["scenario_analysis"]["performance_metrics"]["avg_quality_score"] /
    ...
    def test_statistical_significance_detection(self):
        ...
        quality_diff = abs(basic_metrics["avg_quality_score"] - scenario_metrics["avg_quality_score"])
        token_diff = abs(basic_metrics["avg_token_usage"] - scenario_metrics["avg_token_usage"])
    ...
    def test_consistency_measurement(self):
        ...
        consistency = results["basic_calibrated"]["performance_metrics"]["response_consistency"]
    ...
    def test_sample_size_impact(self):
        ...
        small_consistency = small_results["basic_calibrated"]["performance_metrics"]["response_consistency"]
        large_consistency = large_results["basic_calibrated"]["performance_metrics"]["response_consistency"]
