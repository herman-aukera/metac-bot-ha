...
class TestEndToEndForecasting:
    ...
    async def test_complete_single_question_forecast(self, e2e_bot, mock_real_apis):
        ...
        with patch('aiohttp.ClientSession.post') as mock_openai_post, \
            ...
            mock_metaculus_response.json = Mock(return_value=mock_real_apis["metaculus"])
            mock_metaculus_response.__aenter__ = Mock(return_value=mock_metaculus_response)
    ...
    async def test_ensemble_forecast_workflow(self, e2e_bot, mock_real_apis):
        ...
        with patch('aiohttp.ClientSession.post') as mock_openai_post, \
            ...
            min_pred, max_pred = min(individual_predictions), max(individual_predictions)
    ...
    async def test_batch_forecasting_workflow(self, e2e_bot, mock_real_apis):
        ...
        with patch('aiohttp.ClientSession.post') as mock_openai_post, \
            ...
            questions = [
                ...
                {**mock_real_apis["metaculus"], "id": 12346, "title": "Climate change impact by 2030?"},
                {**mock_real_apis["metaculus"], "id": 12347, "title": "Space colony by 2035?"}
            ]
    ...
    async def test_error_recovery_workflow(self, e2e_bot, mock_real_apis):
        ...
        with patch('aiohttp.ClientSession.post') as mock_openai_post, \
            ...
            mock_openai_response = Mock()
            mock_openai_response.status = 200
            mock_openai_response.json = Mock(return_value={
                "choices": [{
                    "message": {
    ...
    async def test_performance_benchmarks(self, e2e_bot, mock_real_apis):
        ...
        with patch('aiohttp.ClientSession.post') as mock_openai_post, \
            ...
            if "resource_usage" in result["metadata"]:
                assert result["metadata"]["resource_usage"]["memory_mb"] < 1000  # Under 1GB
                assert result["metadata"]["resource_usage"]["api_calls"] <= 5  # Limited API calls
